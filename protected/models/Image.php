...
class Image extends CActiveRecord
{
	...
	{
		return Yii::app()->getBaseUrl(true)."/uploads/img/".$this->path;
	}

	public function getPathOriginal()
	{
		return Yii::getPathOfAlias('webroot')."/uploads/img/".$this->path;
	}

	public function getUrlThumb()
	{
		return Yii::app()->getBaseUrl(true)."/uploads/img/thumbs/".$this->path;
	}

	public function getPathThumb()
	{
		return Yii::getPathOfAlias('webroot')."/uploads/img/thumbs/".$this->path;
	}
	...
	public function saveImage(CUploadedFile $photoFile)
	{
		...
		try {
			...
			if ($image) {
		        ...
		        if ($w > 1000) {
		        	$image->resize(1000, 0, ImageConv::WIDTH);

			        $upload_directory = Yii::getPathOfAlias('webroot') . '/uploads/img/thumbs/'.$this->path;
					$image->save($upload_directory);
				} else {
					copy($this->getPathOriginal(), $this->getPathThumb());
				}
			}
